...
node = ["10.0.1.1","10.0.2.1","10.0.3.1","10.0.4.1","10.0.1.100","10.0.1.200","#10.0.2.100","10.0.2.200","10.0.3.100","10.0.3.200","10.0.4.100","10.0.4.200"]
switch = ["10.0.1.1","10.0.2.1","10.0.3.1","10.0.4.1"]
...
class l3_switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    ...
    if not packet.parsed:
      log.warning("DPID %i PORT %i ignoring unparsed packet", dpid, inport)
      return
    ...
    if isinstance(packet.next, ipv4):
      ...
      if packet.next.srcip in self.arpTable[dpid]:
        if self.arpTable[dpid][packet.next.srcip] != (inport, packet.src):
          log.info("DPID %i PORT %i RE-learned %s", dpid,inport,packet.next.srcip)
      else:
        log.debug("DPID %i PORT %i learned %s", dpid,inport,str(packet.next.srcip))
      ...
      if dstaddr in self.arpTable[dpid]:
        ...
        if prt == inport:
          log.warning("DPID %i PORT %i not sending packet for %s back out of the " +
                      "input port" % (dpid, inport, str(dstaddr)))
        else:
          log.debug("DPID %i PORT %i installing flow for %s => %s out port %i"
      ...
      elif (dstaddr not in self.arpTable[dpid])and (dstaddr not in switch):
	...
	icmp.payload = pay
          ...
          if a.protosrc != 0:
            ...
            if a.protosrc in self.arpTable[dpid]:
              if self.arpTable[dpid][a.protosrc] != (inport, packet.src):
                log.info("DPID %i PORT %i RE-learned %s", dpid,inport,str(a.protosrc))
            else:
              log.debug("DPID %i PORT %i learned %s", dpid,inport,str(a.protosrc))
            ...
            if a.opcode == arp.REQUEST:
              # Maybe we can answer
                ...
                # We have an answer...
                  ...
                  e.payload = r
                  #e.set_payload(r)
                  log.debug("DPID %i PORT %i answering ARP for %s" % (dpid, inport,
                   str(r.protosrc)))
                  msg = of.ofp_packet_out()
                  msg.data = e.pack()
      ...
      log.debug("DPID %i PORT %i flooding ARP %s %s => %s" % (dpid, inport,
       {arp.REQUEST:"request",arp.REPLY:"reply"}.get(a.opcode,
       'op:%i' % (a.opcode,)), str(a.protosrc), str(a.protodst)))
